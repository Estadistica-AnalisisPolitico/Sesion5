...
Estos datos nos dan información sobre ex convictos y el tiempo que demoran en volver a delinquir. Hagamos ajustes según la metadata:
...
```{r}
# efecto nominal
rGauss=lm(h1,data=carcel,contrasts=list(nivelEduca="contr.treatment"))
#
summary(rGauss)
```
...
#
rLogit=glm(h2,data=carcel, family = binomial,contrasts = list(nivelEduca = "contr.treatment"))
#
summary(rLogit)
```
...
La gráfica anterior nos da una idea de cómo se comporta esta población; por ejemplo, la gráfica nos dice que si pasan 40 semanas, la probabilidad de volver está cerca al 80%. 
...
```{r, eval=TRUE}
# forzando a nominal: 
carcel$nivelEduca_N=as.factor(as.numeric(carcel$nivelEduca))
COX_H1= formula(survival~tuvoApoyoDinero+nivelEduca_N+vecesEnCarcel)
