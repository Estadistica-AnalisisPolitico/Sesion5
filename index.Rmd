...
Estos datos nos dan información sobre ex convictos y el tiempo que demoran en volver a la cárcel. Hagamos ajustes a los __tipos de datos__ según la metadata:
...
La \@ref(tab:h1h2) nos muestra el resultado de modelar la H1 con una regresión Gaussiana, y la H2 con una regresión logística binaria:
...
El primer paso para usar EHA, es indicarle a R que trate a la data de esa manera. Creemos una nueva columna _survival_ en nuestra tabla actual.
...
KM es el procedimiento descriptivo básico que se utiliza para ver la dinámica de sobrevivencia. Así, veamos en la Figura \@ref(fig:Kmfig) nos muestra el comportamiento genérico de permanecer libre.
...
Siendo la H0 de KM: no hay diferencias entre grupos, con el p-valor obtenido la diferencia no es significativa al 0.05 (pero sí al 0.1). La Figura \@ref(kmcurveConfInt) aclara por qué.
...
Como toda regresión, esta técnica permite utilizar regresores o predictores o covariados, pero no modela la duración sino el _riesgo de que el evento suceda_ (ser re arrestado). Veamos la Tabla \@ref(tab:coxtable).
...
La Tabla \@ref(tab:coxtable) muestra los coeficientes originales y exponenciados (las razones de riesgo o _hazard ratios_) . Veamos cada uno:

* Se puede inferir que dar financiamiento disminuye el riesgo de ser re arrestado con una significancia del 0.05. La columna a la derecha (los _HRs_) nos da la magnitud promedio de ese riesgo como  la distancia absoluta al uno (_1_); en este caso:
...
* Se puede inferir que haber estado previamente en la cárcel aumenta el riesgo de ser re arrestado con una significancia del 0.01. La magnitud promedio de ese riesgo es la distancia absoluta al uno (_1_); en este caso:

```{r}
(carcelantes=abs(1-exp(coef(rcox1)[6])))
```
De ahí que, podemos sostener que el riesgo de volver a la cárcel para alguien con apoyo financiero se multiplica por el factor `r 100*round(carcelantes,4)` cada vez que aumenta en uno los años de encarcelamientos previos^[Como la Tabla \@ref(tab:coxtable) muestra intervalos de confianza la interpretación puede darse utilizando tales valores. 
].
